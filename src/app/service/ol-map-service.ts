...
export class OlMapService {
   ...
   */
  operationLayers: VectorLayer<VectorSource> = null;
  /**浏览图层，只做一些显示上的操作，不修改操作图层上的图层 */
  viewLayer: VectorLayer<VectorSource> = null;
  ...
  sourceArr = sourceArr
  private otherTest = {
    sourceUrl: this.sourceArr[3].url,
  ...
  addInteractions(type: any, succession: boolean, isGeojson: boolean, imgSrc?: string): Observable<string | Feature<Geometry> | GeoJsonCollectionType> {
    if (type !== 'None') {
      ...
      if (type === 'Square') {
        this.drawType = 'Circle' as GeometryType;
  ...
  addAnimate(feature: Feature<Geometry>) {
    ...
    const duration = 3000;
    // 进行地图水波渲染
    this.animateKeys.push(this.operationLayers.on('postrender', animate.bind(this)) as EventsKey);
   ...
   */
  showClusterMap(features: Feature<Geometry>[], style: ClusterStyle = {}): { source: Cluster, layer: VectorLayer<VectorSource> } {
   ...
   * @returns 地图点击事件key
   */
  clusterClickEven(clustersLayer: VectorLayer<VectorSource>) {
  ...
  editLayer(): Observable<Feature<Geometry>[]> {
    ...
    this.modify.on('modifyend', e => {
      const result = e.features.getArray() as Feature<Geometry>[];
      subject.next(result)
    });
    return subject;
  }
   ...
   * @returns 
   */
  markerAnimation(routeCoords: Array<any>): VectorLayer<VectorSource> {
